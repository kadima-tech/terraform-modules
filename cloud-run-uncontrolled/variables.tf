...
variable "allow_unauthenticed_access" {
  type        = bool
  default     = false
...
variable "loadbalancer_backend_create" {
  type        = bool
  default     = false
  description = "Should this also create a serverless load balancer backend so you can attach this server to a url map?"
}

variable "security_policy" {
  type        = string
  default     = null
  description = "The cloud armour security policy to apply"
}

variable "cpu_throttling" {
  type        = bool
  default     = null
  description = "Specify whether CPU is only allocated during request processing"
}
