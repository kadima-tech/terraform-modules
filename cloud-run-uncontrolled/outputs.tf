...
output "allow_unauthenticed_access" {
  value       = var.allow_unauthenticed_access
...
output "loadbalancer_backend" {
  value       = length(google_compute_backend_service.load-balancer-backend) > 0 ? google_compute_backend_service.load-balancer-backend[0].self_link : ""
  description = "If loadbalancer_backend_create is set to true this will contain the backend"
}
